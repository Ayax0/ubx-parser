...
export function readBitFromUInt16(byte: number, offset = 0, length = 1) {
  if (offset <= 7 && offset + length > 8) {
    const blockA = readBitFromUInt8(
      byte & 0xff,
      offset,
      offset + length > 8 ? 8 - offset : length,
    );
...
export function readBitFromUInt32(byte: number, offset = 0, length = 1) {
  if (offset <= 15 && offset + length > 16) {
    const blockA = readBitFromUInt16(
      byte & 0xff_ff,
      offset,
      offset + length > 16 ? 16 - offset : length,
    );
    const blockB = readBitFromUInt16(
      (byte >> 16) & 0xff_ff,
      0,
      offset + length - 16,
    );
    ...
    return readBitFromUInt16((byte >> 16) & 0xff_ff, offset - 16, length);
  }
}
